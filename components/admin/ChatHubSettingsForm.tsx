...
export function ChatHubSettingsForm({ initial }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <AdminCard title={t('channelsTitle')} description={t('channelsSubtitle')}>
        <div className="space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-slate-900">
                {t('enableWhatsAppLabel')}
              </p>
              <p className="text-xs text-slate-500">
                {t('enableWhatsAppHint')}
              </p>
            </div>
            <Toggle
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-slate-900">
                {t('enableInternalChatLabel')}
              </p>
              <p className="text-xs text-slate-500">
                {t('enableInternalChatHint')}
              </p>
            </div>
            <Toggle
              checked={form.enableInternalChat}
              onChange={(v) => update('enableInternalChat', v)}
              disabled={saving}
            />
          </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-slate-900">
                {t('enableContactFormLabel')}
              </p>
              <p className="text-xs text-slate-500">
                {t('enableContactFormHint')}
              </p>
            </div>
            <Toggle
              checked={form.enableContactForm}
              onChange={(v) => update('enableContactForm', v)}
              disabled={saving}
            />
          </div>
        </div>
      ...
      <AdminCard title={t('whatsappTitle')} description={t('whatsappSubtitle')}>
