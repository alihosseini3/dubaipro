...
import { reorderMegaMenuItems } from '@/lib/header/service';
...
export async function POST(request: Request) {
  ...
  try {
    await reorderMegaMenuItems(ids as string[]);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return handlePrismaError(error, 'POST /api/admin/header/mega/reorder');
  }
}
