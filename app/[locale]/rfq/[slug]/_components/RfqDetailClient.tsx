        ...
        <div className="mx-auto max-w-5xl px-4 py-8 md:px-6 lg:px-8">
          <div className="flex flex-col gap-6 lg:flex-row">
            ...
            <aside className="w-full space-y-4 lg:sticky lg:top-20 lg:h-fit lg:w-64 lg:shrink-0">
              ...
              {currentUserId && rfq.contactWhatsapp && (
                <div className="overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-sm">
                  <div className="border-b border-emerald-100 bg-emerald-50 px-4 py-3">
                    <h3 className="text-[11px] font-bold uppercase tracking-widest text-emerald-700">
                      {isBuyer ? t('contactYours') : t('contactBuyer')}
                    </h3>
                  </div>
                  <div className="space-y-2 px-4 py-4">
                    <a
                      href={`https://wa.me/${rfq.contactWhatsapp.replace(/[^\d]/g, '')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 rounded-xl bg-emerald-500 px-3 py-2.5 text-sm font-bold text-white transition hover:bg-emerald-600"
                    >
                      ...
                      <span dir="ltr" className="truncate">{rfq.contactWhatsapp}</span>
                    </a>
                    {rfq.contactEmail && (
                      <a
                        href={`mailto:${rfq.contactEmail}`}
                        className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-orange-300 hover:text-orange-600"
                      >
                        <svg className="h-4 w-4 shrink-0 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        <span dir="ltr" className="truncate">{rfq.contactEmail}</span>
                      </a>
                    )}
                    {!isBuyer && <p className="text-[11px] leading-snug text-slate-400">{t('contactHint')}</p>}
                  </div>
                </div>
              )}
