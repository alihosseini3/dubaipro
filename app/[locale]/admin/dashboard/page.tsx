...
export default async function AdminDashboardPage({ params }: Props) {
  ...
  const [totalProducts, totalRfqs, totalUsers, totalSuppliers, openRfqs, recentRfqsRaw] =
    await Promise.all([
  ...
  const columns: Column<RecentRfqRow>[] = [
    ...
    { key: 'status', header: t('status'), render: (r) => <StatusBadge status={r.status} variant="rfq" /> },
    {
      key: 'date',
      header: t('createdAt'),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard label={t('totalRfqs')} value={totalRfqs} hint={`${openRfqs} ${t('openRfqs')}`} />
